...
<script>
  ...
  export default {
          ...
          },
          {
            ...
            render: (h, params) => {
              ...
              if(row.lostPicture){
                return h('img', {
                  attrs:{
                    src: 'http://images.mahui53541.com/'+row.lostPicture,
            ...
            render: (h, params) => {
              ...
              const text = row.deleted === true ? '删除' : row.hasPassed === false?"驳回":row.isFound?'找回':'寻找';
              return h('Tag', {
                props: {
                  type: 'dot',
                  color: color
            ...
            render: (h, params) => {
              ...
              return h('div', [
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.axios.delete('/api/lost/' + row.id ).then((res) => {
                        this.getLost()
                      }).catch((err)=>{
                        this.$Message.error({
                          content: '操作失败',
                ...
                h('Button', {
                  ...
                  on: {
                    click: () => {
                      this.axios.put('/api/lost/'+this.user.id+"/"+ row.id ).then((res) => {
                        this.getLost()
                      }).catch((err)=>{
                        this.$Message.error({
                          content: '操作失败',
    ...
    methods: {
      getLost() {
        this.loading = true
        this.axios.get('/api/lost/admin/' + this.page + '/' + this.pageSize, {
